...
exports.login = function(req, res){
	...
	dbconn.query('select * from color', function(err, rows, cols){ //query 함수의 첫 번째 인자에 sql 문을 작성하고 두 번째 인자는 질의문 수행 결과를 처리할 함수를 넘겨준다. 
		   //함수는 db-mysql의 질의문 처리 함수 인자와 동일하다.
		if(err) {
			throw err;
		} else {
		
			var query = dbconn.query("insert into color(r, g, b, a) values(?, ?, ?, ?)", [10, 4, 5, 1], 
		function(){
